...
// Source: core/src/slice/mod.rs:2302-2307 and core/src/slice/iter.rs:557-571
// Source item sha256: 48bb9fbee8197b7c00e12c4f3645c65321c88576ac8730eb5db36302d69b4bdb
// Dependency manifest: proof_manifests/092_core_slice_split_inclusive/dependency_assumption_manifest.json
...
pub closed spec fn slice_iterator_view<'a, T, P>(iter: SplitInclusive<'a, T, P>) -> SliceIteratorView<T>
    where
        P: FnMut(&T) -> bool,
{
    SliceIteratorView {
...
impl<'a, T: 'a, P> SplitInclusive<'a, T, P>
    where
        P: FnMut(&T) -> bool,
{
    pub fn new(slice: &'a [T], pred: P) -> (ret: Self)
        ensures
            slice_predicate_split_view(ret, slice@, pred, true, false, 0),
    {
        proof {
        ...
        let ret = Self { v: slice, pred, finished };
        proof {
...
pub fn split_inclusive<'a, T: 'a, F>(slice: &'a [T], pred: F) -> (iter: SplitInclusive<'a, T, F>)
    where
        F: FnMut(&T) -> bool,
    ensures
        slice_predicate_split_view(iter, slice@, pred, true, false, 0),
{
    SplitInclusive::new(slice, pred)
}
