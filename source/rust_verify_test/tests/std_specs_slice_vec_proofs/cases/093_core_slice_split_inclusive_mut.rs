...
// Source: core/src/slice/mod.rs:2326-2331 and core/src/slice/iter.rs:807-821
// Source item sha256: fdec3a15e77cff14e7dfde0b1da0d454668e2da3e3e8d48d9072573f8a582778
// Dependency manifest: proof_manifests/093_core_slice_split_inclusive_mut/dependency_assumption_manifest.json
...
pub closed spec fn slice_iterator_view<'a, T, P>(
    iter: SplitInclusiveMut<'a, T, P>,
) -> SliceIteratorView<T>
    where
        P: FnMut(&T) -> bool,
{
...
impl<'a, T: 'a, P> SplitInclusiveMut<'a, T, P>
    where
        P: FnMut(&T) -> bool,
{
    pub fn new(slice: &'a mut [T], pred: P) -> (ret: Self)
        ensures
            slice_predicate_split_view(ret, old(slice)@, pred, true, false, 0),
    {
        proof {
...
pub fn split_inclusive_mut<'a, T: 'a, F>(
    slice: &'a mut [T],
    pred: F,
) -> (iter: SplitInclusiveMut<'a, T, F>)
    where
        F: FnMut(&T) -> bool,
    ensures
        slice_predicate_split_view(iter, old(slice)@, pred, true, false, 0),
{
    SplitInclusiveMut::new(slice, pred)
}
